...
void CImageProcessingDoc::OnProcessEqualization()
{
	...
	if (m_pImage) 
	{
		...
		int brightness = 0; // Histogram을 만들기 위하여 brightness 변수 설정

		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				color = m_pImage->GetPixelColor(x, y);
				brightness = ((int)color.rgbBlue + (int)color.rgbGreen + (int)color.rgbRed) / 3;
				sum_hist[brightness]++;
			}
		}
		...
		for(int i=0; i<256 ; i++)
		{
			LUT[i] = (int)(scale_factor * sum_hist[i]);
